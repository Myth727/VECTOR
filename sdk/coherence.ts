...
export function computeCoherence(
  ...
  // V1.7.0: Exponential Bayesian blending — α(t) = 1 − exp(−t/τ)
  // Smooth continuous transition from prior-dominated (early) to signal-dominated (late).
  // Strictly dominates linear ramp — no hard cutoff at turn 10.
  // τ=5 gives similar early-session behavior but with smooth continuous transition.
  // Reference: ChatGPT Cathedral Q1 resolution, April 16 2026.
  const TAU_BLEND = 5.0;
  const alpha_t   = 1 - Math.exp(-ah.length / TAU_BLEND);
  const prior     = 0.75;
  const blended   = alpha_t * raw + (1 - alpha_t) * prior;
